...
def train():
    ...
    param = {'max_depth':5, 'eta':0.5, 'silent':1, 'objective':'reg:linear', 'min_child_weight':1}
    ...
    bst = xgb.Booster({'nthread': 16}, model_file=modelPath)
    ...
    resultPath = '/home/luolaihu/Downloads/test/result'
    if os.path.exists(resultPath):
        os.remove(resultPath)
    with open(resultPath, 'a') as out:
        for i in range(len(preds)):
            out.write('label: %s , predict %s' % (indexMap[i], preds[i]) + '\r\n')
